...
# [QUAN TRỌNG] Hàm này đã được cập nhật nhận đủ 9 tham số
...
def solve_vrp(locations, truck_capacity):
    ...
    depot = locations[0] # Điểm đầu tiên là Kho
    customers = locations[1:] # Các điểm còn lại là khách
    
    routes = [] # Danh sách các chuyến xe (Mỗi chuyến là 1 list điểm)
    current_route = [depot]
    current_load = 0
    current_loc = depot
    total_distance = 0
    
    # Hệ số đường bộ (Ước tính đường thực tế dài hơn đường chim bay 30%)
    ROAD_FACTOR = 1.3 
    ...
    while customers:
        ...
        # Tìm khách hàng gần nhất mà xe vẫn chở được
        for node in customers:
            # Giả sử mỗi đơn hàng nặng ngẫu nhiên hoặc lấy từ DB (Ở đây giả lập weight nếu chưa có)
            # Trong thực tế bạn sẽ lấy weight từ DB. Ở đây mình giả định mỗi điểm là 1 đơn vị hàng.
            item_weight = node.get('weight', 100) # Mặc định 100kg nếu không có info
        ...
        if nearest_node:
            # Nếu tìm được khách gần nhất và xe còn chỗ -> Đi tiếp
            ...
            # Nếu không còn khách nào vừa xe (hoặc hết khách gần) -> Quay về kho
            dist_back = math.sqrt((current_loc['lat'] - depot['lat'])**2 + (current_loc['lon'] - depot['lon'])**2)
            total_distance += dist_back
            current_route.append(depot) # Kết thúc chuyến
            
            routes.append(current_route) # Lưu chuyến này lại
